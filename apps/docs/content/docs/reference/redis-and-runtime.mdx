...
```text
TaskEngine.layer(config?): Layer<TaskEngine, TaskEngineConfigurationError, RedisPool>
```

Requires an ambient `RedisPool`. It does not provide connection roles, health,
Effect Redis, or Crypto. It does not select a process runtime.

`TaskEngine.layerNoDeps` is an alias of `TaskEngine.layer`.
...
```text
NodeLive.layer({ engine?, redis? }): Layer<LiveServices, LiveError>
```
...
live graph (`NodeRedisPool` plus `NodeCrypto`).

Bun plus node-redis is a separate composition:
