...
| `schemaId` | `string` | `name` | Stable identity of the payload, success, failure, and progress schema family. |
...
| `progress` | `Schema.Top` | unset | Opts new generations into custom progress and automatic lifecycle history. `Schema.Never` enables lifecycle-only history. |
...
```text
TaskDefinition<Payload, Success, Error, RetryR, IdentityR, Progress = Schema.Never>
```
...
| `progressSchema` | Optional custom progress schema; presence enables history. |
...
| `maxHistoryEntries` | `null` (unlimited) |

`maxEventEntries` must be at least 1. `maxHistoryEntries` must be null, omitted,
or a positive safe integer; a finite cap trims oldest task-history entries.
Other storage limits and all retention durations must be non-negative safe
integers. History follows task-record disposal and has no separate TTL.
