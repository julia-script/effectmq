...
Separates task creator provenance, explicit result retention, and execution dependency so nested offers do not silently acquire workflow-like lifecycle semantics.

## ADDED Requirements

### Requirement: Creator provenance is automatic and informational
A task offered from a managed task handler SHALL record the running task as its creator. Creator provenance SHALL NOT retain results, order execution, propagate cancellation, or propagate failure.
...
A producer SHALL explicitly request a result-retention hold when it needs a terminal task record to remain readable until a named holder settles. Nested offers SHALL NOT create such a hold by default.
...
A retention hold SHALL be identified by holder and retained task generation. Reacquiring the same hold SHALL have no additional effect, while a different holder SHALL acquire an independent hold.
...
A retention hold SHALL be acquired only for an existing holder that has not settled. Failure to validate any requested holder SHALL leave the retained task and all relationships unchanged.
...
Removing a task generation with active retention holds SHALL fail unless an explicit administrative force mode is used. Settling or removing a holder SHALL release every hold owned by that holder in bounded, resumable batches.
...
The library SHALL NOT delay a holder's completion, cancel a retained task, or translate a retained task's failure solely because a retention hold exists.
...
Public documentation SHALL use creator, spawned task, retention hold, and execution dependency for their distinct meanings. It SHALL use child task only for a future relationship that actually defines structured lifecycle behavior.
