...
describe("TaskEngine locking", () => {
  test("one maintenance pass recovers at most the configured lease batch", () =>
    Effect.gen(function* () {
      ...
      expect(yield* redis.send("ZCARD", `~effectmq:v1:${prefix}:active`)).toBe(
        1,
      );
  ...
  test("a stale token cannot complete, fail, or extend an attempt", () =>
    Effect.gen(function* () {
      const engine = yield* TaskEngine.TaskEngine;
  ...
  test("removeLock voluntarily requeues without recording a stall", () =>
    Effect.gen(function* () {
      const engine = yield* TaskEngine.TaskEngine;
      yield* TaskEngine.setMockTime(1000000000000);
  ...
  test("a previous attempt token cannot mutate a re-acquired task", () =>
    Effect.gen(function* () {
      const engine = yield* TaskEngine.TaskEngine;
      const redis = yield* RedisPool.RedisPool;
      yield* TaskEngine.setMockTime(1000000000000);
      ...
      expect((yield* engine.getTask(prefix, "fenced"))?.errors).toHaveLength(1);
  ...
  test("attempt, handler failure, and stalled counts are independent and stalls terminate", () =>
    Effect.gen(function* () {
      const engine = yield* TaskEngine.TaskEngine;
      const redis = yield* RedisPool.RedisPool;
  ...
  test("error history retains only the configured newest entries", () =>
    Effect.gen(function* () {
      const engine = yield* TaskEngine.TaskEngine;
  ...
  test("non-debug lease deadlines come from Redis server time", () =>
    Effect.gen(function* () {
      const redis = yield* RedisPool.RedisPool;
      ...
      const before = Number(redisTime[0]) * 1000 + Number(redisTime[1]) / 1000;
