...
layer(TestLayer, { excludeTestServices: true, timeout: "60 seconds" })(
  "TaskEngine result-retention relationships (real Redis time)",
  (it) => {
    it.effect(
      "replaying the same holder/retained generation creates one hold",
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      "independent live holders can retain the same existing generation",
      () =>
        Effect.gen(function* () {
          const engine = yield* TaskEngine.TaskEngine;
          const redis = yield* RedisPool.RedisPool;
          ...
          expect(
            (yield* engine.getTask(retainedQueue, "retained"))?.outcome,
          ).toBe("success");

          yield* succeedNext(engine, "retention-holder-a");
          expect(
            yield* engine.getTask(retainedQueue, "retained"),
          ).not.toBeNull();
    ...
    it.effect(
      "a settled holder is rejected without creating the retained task",
      () =>
        Effect.gen(function* () {
          const engine = yield* TaskEngine.TaskEngine;
    ...
    it.effect(
      "holder removal releases its hold without affecting runnable work",
      () =>
        Effect.gen(function* () {
          const engine = yield* TaskEngine.TaskEngine;
          const redis = yield* RedisPool.RedisPool;
    ...
    it.effect(
      "terminal settlement is visible immediately while delete waits for holds",
      () =>
        Effect.gen(function* () {
          const engine = yield* TaskEngine.TaskEngine;
    ...
    it.effect("mark policy indexes a held terminal task immediately", () =>
      Effect.gen(function* () {
        const engine = yield* TaskEngine.TaskEngine;
    ...
    it.effect(
      "ordinary removal rejects holds while force removal is explicit",
      () =>
        Effect.gen(function* () {
          const engine = yield* TaskEngine.TaskEngine;
    ...
    it.effect(
      "large holder release leaves and drains a durable bounded continuation",
      () =>
        Effect.gen(function* () {
...
layer(TestLayer, { excludeTestServices: true, timeout: "60 seconds" })(
  "TaskEngine creator provenance (real Redis time)",
  (it) => {
    it.effect(
      "creator identity is immutable metadata and survives creator removal",
      () =>
        Effect.gen(function* () {
          const engine = yield* TaskEngine.TaskEngine;
