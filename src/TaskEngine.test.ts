...
describe("TaskEngine", () => {
  test("rejects maintenance batches above the supported atomic bound", () =>
    Effect.gen(function* () {
      const exit = yield* Effect.exit(
        TaskEngine.make({ maintenanceBatchSize: 1_001 }),
      );
      expect(exit._tag).toBe("Failure");
    }).pipe(TestRuntime.runPromise));

  test("task inspection is capped, cursor-paginated, and ordered", () =>
    Effect.gen(function* () {
  ...
  test("one maintenance pass promotes at most the configured due batch", () =>
    Effect.gen(function* () {
      ...
      expect(yield* redis.send("LLEN", `~effectmq:v1:${prefix}:wait`)).toBe(2);
      ...
      expect(
        yield* Metric.value(
          Metric.withAttributes(Observability.dueBacklog, { queue: prefix }),
        ),
      ).toMatchObject({ value: 1 });
    }).pipe(TestRuntime.runPromise));

  test("terminal records, results, indexes, and dead letters expire independently", () =>
    Effect.gen(function* () {
      const engine = yield* TaskEngine.TaskEngine;
      const redis = yield* RedisPool.RedisPool;
  ...
  test("retention cleanup leaves bounded due work for later sweeps", () =>
    Effect.gen(function* () {
      const engine = yield* TaskEngine.make({
        debugMode: true,
        maintenanceBatchSize: 2,
      ...
      expect(yield* redis.send("ZCARD", `~effectmq:v1:${prefix}:success`)).toBe(
        0,
      );
      expect(
      ...
      expect(
        yield* redis.send("ZCARD", `~effectmq:v1:${deadPrefix}:dead-letter`),
      ).toBe(0);
      expect(
        yield* redis.send(
          "ZCARD",
  ...
  test("corrupt non-list error history fails instead of normalizing to empty", () =>
    Effect.gen(function* () {
      const engine = yield* TaskEngine.TaskEngine;
      const redis = yield* RedisPool.RedisPool;
      ...
      const error = yield* engine.getTask(prefix, "corrupt").pipe(Effect.flip);
      expect(error._tag).toBe("TaskEngineError");
      expect(error.message).toBe("Failed to decode task");
    }).pipe(TestRuntime.runPromise));
  test("cached scripts recover independently after SCRIPT FLUSH", () =>
    Effect.gen(function* () {
      const redis = yield* RedisPool.RedisPool;
  ...
  test("re-offering a waiting task preserves exactly one wait membership", () =>
    Effect.gen(function* () {
  ...
  test("renewing a lease preserves exactly one active membership", () =>
    Effect.gen(function* () {
      const taskEngine = yield* TaskEngine.TaskEngine;
      const redis = yield* RedisPool.RedisPool;
  ...
  test("Canceled error with a scheduled retry still skips retrying", () =>
    Effect.gen(function* () {
      ...
      yield* writeError(taskEngine, prefix, "c2", canceled(now), now + 5000);
  ...
  test("structured payloads round-trip through msgpack unchanged", () =>
    Effect.gen(function* () {
      const taskEngine = yield* TaskEngine.TaskEngine;
