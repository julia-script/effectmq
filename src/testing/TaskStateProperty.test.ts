...
import { TestRuntime } from "./redisLayer.js";
...
describe("generated Redis state-machine sequences", () => {
  test(
      ...
      Effect.gen(function* () {
        ...
        for (let seed = 1; seed <= 32; seed++) {
          ...
          const assertRedisInvariants = Effect.fnUntraced(function* () {
            ...
            expect(stored?.handlerFailureCount).toBe(task.handlerFailureCount);
            expect(stored?.stalledAttemptCount).toBe(task.stalledAttemptCount);
          ...
          while (!terminal && operations < 20) {
            ...
            if (task.state === "waiting" || task.state === "retry-scheduled") {
              ...
              if (choice < 0.18 && renewals < 2) {
                ...
                model = value(Model.expire(model, { id, maxStalledCount: 2 }));
              } else if (choice < 0.9) {
                yield* engine.writeSuccess(prefix, id, task.leaseToken ?? "", {
                  ok: true,
                });
                model = value(Model.succeed(model, id, task.leaseToken ?? ""));
                terminal = true;
              } else {
